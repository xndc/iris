// NOTE(xndc): This is an old snapshot of Emil Ernerfeldt's loguru library with some changes. It
// uses stb_sprintf instead of the system vsnprintf and builds under Emscripten.  There are probably
// other changes (formatting?) but I don't remember exactly what they are.
...
#ifndef LOGURU_USE_STB_SPRINTF
	#define LOGURU_USE_STB_SPRINTF 1
#endif

#if LOGURU_USE_STB_SPRINTF
...
// ------------------------------------------------------------------------------
	...
	static Text vtextprintf(const char* format, va_list vlist)
	{
		...
		int written = stbsp_vsnprintf(buff, size, format, vlist);
		while (written == size) {
			size *= 2;
			buff = (char*) realloc(buff, size);
			written = stbsp_vsnprintf(buff, size, format, vlist);
		}
		return Text(buff);
#else
#ifdef _WIN32
	...
	*/
	...
	static void print_preamble(char* out_buff, size_t out_buff_size, Verbosity verbosity, const char* file, unsigned line)
	{
		...
		if (g_preamble_uptime && pos < out_buff_size) {
			int bytes = snprintf(out_buff + pos, out_buff_size - pos, "%.3fs ", uptime_sec);
			if (bytes > 0) {
				pos += bytes;
			}
		}
		if (g_preamble_thread && pos < out_buff_size) {
			int bytes = snprintf(out_buff + pos, out_buff_size - pos, "[%s]", thread_name);
			if (bytes > 0) {
				pos += bytes;
			}
		}
		if (g_preamble_file && pos < out_buff_size) {
			...
			int bytes = snprintf(out_buff + pos, out_buff_size - pos, " %s:%u ", shortened_filename, line);
			if (bytes > 0) {
				pos += bytes;
			}
		}
